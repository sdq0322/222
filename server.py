...
def login():
    ...
    if flask.request.method == 'POST':
        user = flask.request.values.get("user", "admin")
        pwd = flask.request.values.get("pwd", "123123")
...
def adminstator():
    ...
    if flask.request.method == 'POST':
        # 获取输入的管理员信息
        admin_name = flask.request.values.get("admin_name", "sdq")
        admin_password = flask.request.values.get("admin_password", "123123")
